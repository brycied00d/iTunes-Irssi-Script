...
##| 0.2.2: Fixed NOTICE/MSG behaviour between channels and PM's. (brycec)
##|

$VERSION = '0.2.2';
%IRSSI = (
    authors     => 'Christopher R. Ungeheier, Bryce Chidester',
    contact     => 'christopher@ungeheier.com, bryce@cobryce.com',
    name        => 'itunes',
    description => 'iTunes script using Applescripting ',
    license     => 'If you modify this script please give me credit.',
    url         => 'http://www.cobryce.com/itunes-irssi-script',
    changed     => '10:52:10 Friday, June 18 2010',
);
...
sub event_privmsg 
{
	...
	# Channel
	#09:06:21 DEBUG: $server = Irssi::Irc::Server=HASH(0x8b9100)
	#09:06:21 DEBUG: $data = #devious :!itunes
	#09:06:21 DEBUG: $address = brycec@unaffiliated/cobryce
	#09:06:21 DEBUG: $nick = brycec_devious
	#09:06:21 DEBUG: $target = #devious
	#09:06:21 DEBUG: $text = !itunes
	# -- Should msg/notice to $target (use $server-ischannel($target))
	# PM
	#09:03:57 DEBUG: $server = Irssi::Irc::Server=HASH(0x89ac50)
	#09:03:57 DEBUG: $data = brycec_work :!itunes
	#09:03:57 DEBUG: $address = brycec@unaffiliated/cobryce
	#09:03:57 DEBUG: $nick = brycec_devious
	#09:03:57 DEBUG: $target = brycec_work
	#09:03:57 DEBUG: $text = !itunes
	# -- Should msg/notice to $nick (use $server-ischannel($target))
	...
	if (Irssi::settings_get_str($IRSSI{'name'} . '_reply') =~ /yes/i)
	{
		if ($text eq '!itunes')
		{
			...
			if ($track eq "" || $track eq "false" || $status eq "stopped")
			{
				...
				# 100617 Sending a NOTICE -> target, only works in channels ($targets)
				#        Sending a MSG will PM the sender ($nick)
				if (Irssi::settings_get_str($IRSSI{'name'} . '_reply_type') =~ /notice/i)
				{
					# If $target is a channel, message the whole channel. Else, $target will be self, so send to $nick instead.
